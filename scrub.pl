...
%options = (
    ...
    "H|host"        => [ \$host,        "Apply host, domain and fqdn format scrubbing (same as -odF). This will unfortunately scrub Java stack traces of class names also, in which case you should not use --domain/--fqdn or --host/--all which include them. Instead use --custom and put your domain regex in scrub_custom.conf" ],
    "o|hostname"    => [ \$hostname,    "Apply hostname format scrubbing (only works on \"<host>:<port>\" otherwise this would match everything (consider using --custom in addition to remove other references)" ],
...
sub skip_java_exceptions($$;$){
    ...
    if($skip_java_exceptions){
        return 1 if $string =~ /(?:^\s+at|^Caused by:)\s+\w+(?:\.\w+)+/ and debug "skipping $name\\sat|^Caused by";
        return 1 if $string =~ /\($regex):[\w-]+\(\d+\)\)/              and debug "skipping $name(regex):\\w(\\d+)";
        return 1 if $string =~ /\$\w+\($regex:\d+\)/                    and debug "skipping $name\$\\w+(regex)";
    }
    return 0;
}
