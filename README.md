...
- ```scrub.pl``` - anonymizes your configs / logs for pasting online for debugging help. Replaces hostnames/domains/FQDNs, email addresses, IP + MAC addresses, Cisco/Juniper passwords, shared keys and SNMP strings, as well as taking a  configuration file of custom phrases such your name, your company's name and other custom phrases such as your project/database/table conventions etc. Each replacement is replaced with a placeholder saying what type of thing was replaced, and there is even an --ip-prefix switch to leave the last IP octect to aid in cluster debugging so vendors/online helpers can still see differentiated nodes communicating with each other in logs
