...
- ```sqlcase.pl / *case.pl``` - corrects capitalization of SQL keywords in files or stdin, plus Cassandra CQL, Couchbase N1QL, Pig Latin, Neo4j, Docker keywords etc. SQL-like dialects supported include Hive, Impala, Cassandra CQL, Couchbase N1QL, MySQL, PostgreSQL, Apache Drill, Microsoft SQL Server and Oracle. More specific ```*case.pl``` command calls limit case rewriting to the targeted platform for tighter control to avoid recasing things that may be keywords in other SQL[-like] dialects
